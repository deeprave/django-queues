...
or before the queue's authoritative current time, SHALL be eligible
immediately. The entry SHALL remain `queued` until it is normally dispatched.

#### Scenario: Enqueue future work
- **WHEN** a caller enqueues an identified entry with a future availability instant
- **THEN** the entry is durable, remains `queued`, and no worker can claim it
  before that instant

#### Scenario: Enqueue immediately available work
- **WHEN** a caller omits the availability instant or supplies one at or before
...
- **WHEN** a worker checks a queue containing both a future entry and an
  immediately available entry
- **THEN** it can dispatch the immediately available entry without claiming the
  future entry
...
instant, it SHALL select higher priority first; remaining ties use the backend's
ordinary arrival order.
